...
export async function getPlatformKPIs() {
  ...
  try {
    ...
    // Fallback: Calculate KPIs manually
    console.log('RPC function not available, calculating KPIs manually')
    
    // Get all businesses
    const { data: businesses } = await adminClient
      .from('businesses')
      .select('*, plans(base_price)')
    ...
    // Calculate MRR
    const mrr = businesses?.reduce((sum, business) => {
      if (business.subscription_status === 'active' && business.plans) {
        return sum + (business.plans.base_price || 0)
      }
      return sum
    }, 0) || 0
    ...
    const churnedBusinesses = businesses?.filter(b => 
    ...
    const growthRate = lastMonthBusinesses > 0 
      ? ((newBusinesses / lastMonthBusinesses) * 100).toFixed(1)
      : 0
