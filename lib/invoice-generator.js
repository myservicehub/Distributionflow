...
export async function generateAndSendInvoice(orderId) {
  try {
    ...
    const downloadUrl = `${baseUrl}/api/invoices?order_id=${orderId}&action=view`
    ...
    if (retailer?.owner_email) {
      ...
      if (emailResult.success) {
        ...
        result.emailAddress = retailer.owner_email
      }
    }

    // Generate WhatsApp share URL (always available for manual sharing)
    const phone = retailer?.phone || retailer?.phone_number
    if (phone) {
      const cleanPhone = phone.replace(/[^0-9]/g, '')
      const message = `📄 Invoice ${invoiceResult.invoiceNumber}\n\n` +
      ...
      result.deliveryMethods.push('whatsapp')
      result.retailerPhone = cleanPhone
    }

    // Always include download option
    result.deliveryMethods.push('download')
