...
import { ChevronDown, ChevronUp, TrendingDown, BarChart3, Package } from 'lucide-react'
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="debt" className="space-y-6">
        <TabsList className="bg-white border border-neutral-200 p-1">
          <TabsTrigger value="debt" className="data-[state=active]:bg-primary-500 data-[state=active]:text-white">Debt Aging</TabsTrigger>
          <TabsTrigger value="sales" className="data-[state=active]:bg-primary-500 data-[state=active]:text-white">Sales by Rep</TabsTrigger>
          <TabsTrigger value="inventory" className="data-[state=active]:bg-primary-500 data-[state=active]:text-white">Inventory Report</TabsTrigger>
        </TabsList>
            ...
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {debtAging.map((item) => (
                      <TableRow key={item.shop_name} className="hover:bg-neutral-50 transition-colors duration-150">
                ...
                <Table>
                  ...
                  <TableBody>
                    {salesByRep.map((rep, idx) => (
                      <>
                        <TableRow key={`${rep.name}-${idx}`} className="cursor-pointer hover:bg-neutral-50 transition-colors duration-150">
                          <TableCell>
                            <Button 
                              variant="ghost" 
                              size="sm"
                              onClick={() => toggleRepExpand(rep.name)}
                              className="p-1 h-auto hover:bg-primary-100 rounded-lg"
                            >
                          ...
                          <TableCell className="font-semibold text-primary-600">{rep.items || 0}</TableCell>
                        ...
                        {expandedReps[rep.name] && rep.products && rep.products.length > 0 && (
                          <TableRow key={`${rep.name}-details-${idx}`}>
                            <TableCell colSpan={6} className="bg-neutral-50 border-t border-neutral-200 p-6">
              ...
              <CardContent className="pt-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-primary-50 border border-primary-200 p-6 rounded-xl">
                    <p className="text-sm text-primary-700 font-medium mb-2">Total Products</p>
                    <p className="text-3xl font-bold text-primary-600">{inventory.length}</p>
                  </div>
                  <div className="bg-success-50 border border-success-200 p-6 rounded-xl">
                    <p className="text-sm text-success-700 font-medium mb-2">Total Stock Value</p>
